...
from gameElements import tetris_pieces_trimmed, NUM_ROW, NUM_COL
...
def find_best_move(parameters, current_board, current_piece, next_pieces, held_piece, combo, b2b, pruning_moves,
                   pruning_breadth,
                   mp_pool):
    all_moves = []
    ...
    if held_piece == "T":
        switch_extra -= parameters[0]
    if current_piece == "T":
        switch_extra += parameters[1]
    ...
    for new_score, position, rotations, extra_score, new_combo, new_b2b, new_board in _find_best_move(
            (current_board, current_piece, combo, b2b, parameters)
    ):
        all_moves.append([
    ...
    if held_piece != current_piece:  # no point to switch
        for new_score, position, rotations, extra_score, new_combo, new_b2b, new_board in _find_best_move(
                (current_board, held_piece, combo, b2b, parameters)
        ):
            all_moves.append([
    ...
    while next_pieces:
        ...
        for idx, _ in enumerate(all_moves):
            ...
            for _, board, _, combo, b2b, held_piece in all_moves[idx][1]:
                map_args.append((board, current_piece, combo, b2b, parameters))
                if held_piece != current_piece:  # no point to switch
                    map_args.append((board, held_piece, combo, b2b, parameters))
...
def get_all_possible_moves(parameters, piece, board, board_terrain, b2b):
    if piece == "T":  # t-spin moves
            ...
            if num_clear_rows > 0:
                if b2b:
                    if actual_lines == expected_lines:
                        extra_score += parameters[2] + parameters[3] * actual_lines * actual_lines
                    else:
                        extra_score += parameters[2] // 3 + parameters[3] // 2.5 * actual_lines
                else:
                    if actual_lines == expected_lines:
                        extra_score += parameters[4] + parameters[5] * actual_lines * actual_lines
                    else:
                        extra_score += parameters[4] // 4 + parameters[5] // 5 * actual_lines
                new_b2b = b2b + 1
            else:
                new_b2b = b2b
        ...
        for rot, pos, _, _, _, blocks in get_mini_t_slots(board, board_terrain):
            ...
            if num_clear_rows > 0:
                if b2b:
                    extra_score += parameters[6] * num_clear_rows
                else:
                    extra_score += parameters[7] * num_clear_rows
                new_b2b = b2b + 1
            else:
                new_b2b = b2b
            yield pos, rot, extra_score, num_clear_rows, new_b2b, new_board
        ...
        for rot, pos, blocks in slots:
            ...
            extra_score = 0
            if num_clear_rows > 0:
                extra_score += parameters[8]
                if ALL_SPIN_B2B:
                    if b2b:
                        extra_score += parameters[9] * num_clear_rows
                    else:
                        extra_score += parameters[10] * num_clear_rows
                    new_b2b = b2b + 1
                else:
                    new_b2b = 0
            else:
...
def _find_best_move(args):
    current_board, current_piece, combo, b2b, parameters = args
    scores = []
    board_terrain = _get_board_terrain(current_board)

    for position, rotations, extra_score, num_clear_rows, new_b2b, new_board in get_all_possible_moves(parameters,
                                                                                                       current_piece,
                                                                                                       current_board,
                                                                                                       board_terrain,
                                                                                                       b2b):
        ...
        if num_clear_rows > 0:
            ...
            new_combo = 0
        score = evaluate_board(parameters, new_board)
        if score > 8000:  # perfect situation
            return (score, position, rotations, score, combo + 1, new_b2b, new_board),

        if num_clear_rows == 4:
            if b2b:
                return (score + parameters[11], position, rotations, parameters[11], combo + 1, new_b2b,
                        new_board),
            return (score + parameters[12], position, rotations, parameters[12], combo + 1, new_b2b,
                    new_board),
        if new_combo > 2:
            extra_score += parameters[13]
        if new_combo > 6:
            extra_score += parameters[14]
        if new_combo > 17:
            extra_score += parameters[15]

        if b2b > new_b2b:  # lost b2b
            extra_score -= parameters[22]
...
def get_positions(board_terrain, piece_terrain):
    return [(max(board_terrain[x + i] - ht for i, ht in enumerate(piece_terrain)), x) for x in
            range(NUM_COL - len(piece_terrain) + 1)]


def place_piece(board, piece_shape, position):
    board = board.copy()
    try:
        board[position[0]:position[0] + piece_shape.shape[0],
        position[1]:position[1] + piece_shape.shape[1]] += piece_shape
    except ValueError:
        return None
    return board
...
def evaluate_board(parameters, board):
    score = 0
    ...
    #ML THIS
    score -= parameters[23] * i_slot_count * i_slot_count  # prevent multiple i slot
    score -= parameters[24] * i_slot_count_too_high
    score -= parameters[25] * tower_count
    ...
    if board_terrain_sorted[-1] >= 12:
        score -= parameters[28] * board_terrain_sorted[-1]
    if board_terrain_sorted[-1] >= 14:
        score -= parameters[29] * board_terrain_sorted[-1]
    if board_terrain_sorted[-1] >= 16:
        score -= parameters[30] * board_terrain_sorted[-1] * board_terrain_sorted[-1]

    score -= parameters[31] * (board_terrain_sorted[-1] - board_terrain_sorted[1])  # second lowest
    diff_for_highest_2 = board_terrain_sorted[-1] - board_terrain_sorted[-2]
    score -= parameters[32] * diff_for_highest_2 * diff_for_highest_2 * (
                diff_for_highest_2 - 1)  # make sure no single high column

    # try to make every column lower
    for h in board_terrain_sorted:
        if h > 15:
            score -= parameters[33] * h * h
        elif h > 10:
            score -= parameters[34] * h * (h - parameters[35])
        else:
            score -= parameters[36] * h
    ...
    for _, pos, max_lines, expected_lines, actual_lines, _ in t_slots:
        if max_lines == 3 and board_terrain_sorted[-1] < TRIPLE_T_SPIN_MAX_HEIGHT:
            triple_count += 1
            score += parameters[16] + actual_lines * parameters[17]
        score += parameters[18] * actual_lines - parameters[19] * (max_lines - expected_lines)
    if triple_count > 1:
        score -= parameters[37] * triple_count
    if max(board_terrain) < T_SPIN_MAX_HEIGHT and len(t_slots) == 1:  # try to make t-spin from scratch
        score += parameters[20] * t_slots[0][3]
